...
class UserShell
{
    ...
    {
        $cpuCores = self::shellExec('grep -P "^processor" /proc/cpuinfo | wc -l');
    ...
    public static function createTrfficsLogFile($filePath)
    {
        ...
        self::shellExec("sudo nethogs -j -v3 -c6 > $filePath");
    ...
    public static function updateUdpPort($newPort)
    {
        ...
        self::shellExec($command);
        self::shellExec("sudo systemctl restart videocall");
    ...
    public static function updateDomainUrl($newUrl)
    {
        ...
        // }
        // $panelPort  = getenv("PORT_PANEL");
        // $newUrl     = $parsedUrl['scheme'] . '://' . $parsedUrl['host'];
        $newUrl     = "$newUrl/cron/master";
        $scriptPath = "/var/www/html/test.sh";

        $sedCommand = "sed -i -E \"s|'http[s]?://[^']+'|'$newUrl'|g\" $scriptPath";
